...
contract EventParticipant {
  ...
  function EventParticipant() public {
  ...
  function  readEventByIndex(uint index) constant public returns (address,uint,
      //generated fields
      string) {
    if(index >= eventArray.length) return;
 ...
 // Updates Event 
  function updateEvent(bytes32 id, 
    ...
    if (eventMap[id].owner != msg.sender) return false; //only event owner can update      
  ...
  function deleteEvent  (bytes32 id) public  returns (bool) {
    ...
    if (eventMap[id].owner != msg.sender) return false; //only event owner can delete      
